-- Les métriques de production s'appuient désormais sur l'origine HTTP vérifiée
-- par l'Edge Function. Les anciens événements ne sont conservés que pour les
-- deux chemins historiques qui étaient réellement exposés au public.
...
with eligible_conversations as (
  ...
  where conversation.metadata ->> 'origin' in (
    ...
    or (
      coalesce(conversation.metadata ->> 'origin', '') = ''
      and coalesce(conversation.metadata ->> 'page', '') in ('/', '/concept-evan.html')
    )
),
sessions as (
  select
...
comment on view private.evan_conversion_daily is
  'Conversions quotidiennes attribuées aux domaines de production vérifiés côté serveur.';
...
comment on view private.sebastien_performance_daily is
  'Pilotage de Sébastien limité aux domaines de production vérifiés côté serveur.';
