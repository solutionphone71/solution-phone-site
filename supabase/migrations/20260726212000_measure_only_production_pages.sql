-- Ne conserve dans les indicateurs que les pages réellement publiées.
-- Les scénarios internes utilisent des libellés sans slash ou des chemins /audit et /test.
...
with eligible_conversations as (
  ...
  where coalesce(conversation.metadata ->> 'page', '') like '/%'
    and coalesce(conversation.metadata ->> 'page', '') not like '/audit%'
    and coalesce(conversation.metadata ->> 'page', '') not like '/test%'
),
sessions as (
  select
...
comment on view private.evan_conversion_daily is
  'Conversions quotidiennes limitées aux chemins des sites publiés ; tests et audits exclus.';
...
comment on view private.sebastien_performance_daily is
  'Pilotage de Sébastien sur trafic de production : conversion, qualité et coût IA.';
